...
# 로그인 관련 설정
###################
# 로그인 안한 사용자가 로그인해야 실행할 수 있는 View를 호출 했을때 이동할 url 설정.
LOGIN_URL = "/account/login"

# 로그인/로그아웃 처리후에 이동할 url - class기반view를 사용할 때 필요.
LOGIN_REDIRECT_URL = "/"
LOGOUT_REDIRECT_URL = "/"

#####################################################################
# static 파일 (html에서 사용할 image, js, css, .. 파일들..)설정
# 1. app/static 아래 저장. -> static 파일들을 자동으로 장고서버가 인식함.
# 2.  1 이외의 경로에 static 파일을 저장 -> setting.py에 설정.
#####################################################################
# static파일을 client에서 요청할 떄 사용할 시작 url의 시작 path.
...
## app/static, staticfiles_dirs 디렉토리들에 있는 static 파일들을 STATIC_ROOT 디렉토리 아래로 모아준다.

# - 운영환경
## http 서버 + wsgi(django실행환경) 나눠서 서버를 구축할 때,
## http 서버가 static 파일 요청을 처리.
## static 파일들을 경로를 http 서버에게 알려줘야 한다.
